#!/bin/bash
# 使用 8 张 GPU 并行为 ILI 生成适配 seq_len=36 的 qwen3 embeddings
...
dataset = mod.get_dataset("ILI", split_name, 36, 24)
print(len(dataset))
PY
}
...
echo "=========================================="
echo "ILI 八卡并行生成 qwen3 embeddings"
...
# ILI 很小，6 张卡切 train，1 张卡跑 val，1 张卡跑 test
TRAIN_GPUS=6
...
echo "等待所有 ILI embedding 生成任务完成..."
for pid in "${PIDS[@]}"; do
    wait "${pid}"
done

echo "=========================================="
echo "✅ ILI embeddings 生成完成"
