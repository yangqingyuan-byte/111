#!/bin/bash
# T3Time_FreTS_FusionExp ILI 八卡并行消融实验脚本
# 用途：按每个 pred_len 的最佳参数组合作为 base config，再改变关键模块开关做消融
...
LOG_DIR="/root/0/T3Time/Results/T3Time_FreTS_FusionExp_Ablation/ILI"
...
MODEL_ID="T3Time_FreTS_FusionExp_Ablation_ILI"
RUN_TAG="${RUN_TAG:-$(date +%Y%m%d_%H%M%S)_$$}"
PROGRESS_DIR="/root/0/T3Time/Results/progress_ablation_ILI_${RUN_TAG}"

BEST_CONFIG_JSON=${BEST_CONFIG_JSON:-$(ls -t /root/0/T3Time/Results/best_config_exports/ILI_all_models_mse_*.json 2>/dev/null | head -n1)}

# 默认使用导出文件里每个 pred_len 对应的最佳 seed；
# 如需统一改成其他 seed，可在运行前设置:
#   SEEDS_STR="2025 2026" bash scripts/T3Time_FreTS_FusionExp/八卡消融ILI.sh
SEEDS_STR="${SEEDS_STR:-}"
USE_EXPORTED_SEED=1
if [ -n "${SEEDS_STR}" ]; then
    USE_EXPORTED_SEED=0
...
check_embeddings_ready() {
    local embed_root
    embed_root=$(python - <<'PY' "${BEST_CONFIG_JSON}"
import json, sys
path = sys.argv[1]
data = json.load(open(path, 'r', encoding='utf-8'))
embed_version = data[0]['params'].get('embed_version', 'qwen3_0.6b_ili36')
print(f"/root/0/T3Time/Embeddings/ILI/{embed_version}")
PY
)
...
fields = [
    str(params.get('seq_len', 36)),
    str(params.get('channel', 64)),
    str(params.get('batch_size', 16)),
    str(params.get('learning_rate', 1e-4)),
    str(params.get('dropout_n', 0.1)),
    ...
    str(params.get('embed_version', 'qwen3_0.6b_ili36')),
    str(params.get('frets_scale', 0.018)),
    str(params.get('sparsity_threshold', 0.009)),
    str(params.get('fusion_mode', 'gate')),
    str(int(bool(params.get('use_frets', True)))),
    str(int(bool(params.get('use_sparsity', True)))),
    str(int(bool(params.get('use_improved_gate', True)))),
    str(params.get('seed', 2025)),
]
print("|".join(fields))
PY
}
...
for pred_len in "${PRED_LENS[@]}"; do
    if [ "${USE_EXPORTED_SEED}" -eq 1 ]; then
        base_seed=$(get_base_config "${pred_len}" | awk -F'|' '{print $20}')
        for ablation_cfg in "${ABLATION_CONFIGS[@]}"; do
            experiments+=("${pred_len}|${ablation_cfg}|${base_seed}")
        done
    else
...
if [ -z "${BEST_CONFIG_JSON}" ] || [ ! -f "${BEST_CONFIG_JSON}" ]; then
    echo "❌ 未找到最佳配置导出文件，请先运行筛选脚本并导出 ILI 的最佳组合"
    exit 1
fi
...
if [ "$#" -eq 0 ]; then
    ...
    echo "=========================================="
    echo "检测到未指定参数，启用 ILI 八卡消融自动并行模式"
    echo "数据集: ${DATA_PATH}"
    echo "预测长度: ${PRED_LENS[*]}"
    echo "最佳配置文件: ${BEST_CONFIG_JSON}"
    echo "消融配置数: ${#ABLATION_CONFIGS[@]}"
    if [ "${USE_EXPORTED_SEED}" -eq 1 ]; then
        echo "种子策略: 使用导出文件中每个 pred_len 的最佳 seed"
    else
...
run_experiment() {
    ...
    IFS='|' read -r SEQ_LEN CHANNEL BATCH_SIZE LEARNING_RATE DROPOUT_N WEIGHT_DECAY E_LAYER D_LAYER HEAD LOSS_FN LRADJ PATIENCE EMBED_VERSION BASE_FRETS_SCALE BASE_SPARSITY_THRESHOLD BASE_FUSION_MODE BASE_USE_FRETS BASE_USE_SPARSITY BASE_USE_IMPROVED_GATE BASE_SEED <<< "$(get_base_config "${PRED_LEN}")"
...
echo "=========================================="
echo "✅ GPU${GPU_ID} 分配的 ILI 消融实验完成"
