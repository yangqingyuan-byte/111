...
为了更清晰地体现小波包分支的实际贡献，本章建议设置三类对比对象：第一类为原始基座模型，用于说明相对于基础频域建模方式的小波包增强是否有效；第二类为去掉小波包分支、仅保留时域与语义对齐的统一框架，用于隔离多尺度频域建模本身的作用；第三类可包含第五章的 FreTS 版本或其他频域基线，用于从不同频域建模策略的角度观察指标差异\cite{yi2023frets,zhou2022fedformer}。训练过程使用统一的优化器、学习率、batch size 和早停策略，并建议在每组实验中重复多次取平均结果，以减小随机初始化带来的波动。

\subsection{MSE 指标表现及显著性分析}

本节重点考察小波包增强模型在 MSE 指标上的表现，并将结果与原始基座模型、无小波包的统一框架以及其他频域建模方法进行比较。结果表建议按照“数据集 × 预测长度 × 方法”的形式组织，同时给出 MSE 和 MAE 两项指标，但分析重点放在 MSE 上，以突出本章方法对大误差样本的抑制能力\cite{hyndman2006accuracy,chai2014rmsemae}。若实验覆盖多个预测长度，可重点关注长预测场景下的指标变化，因为在此类设置中，多尺度建模对误差累积的缓解作用通常更加明显。  

本章主要关注小波包多尺度分解对大误差样本的抑制能力，因此结果分析更应围绕 MSE 优势、极端波动片段的拟合效果以及分解层数的影响展开。若实验结果表明该方法在多数数据集和长预测步长设置下取得更低的 MSE，则可说明局部频带建模与选择性融合确实增强了模型对尖峰、突变和局部异常模式的表征能力\cite{coifman1992bestbasis,chai2014rmsemae}。

在结果分析中，应重点讨论以下现象：其一，小波包增强模型是否在多数数据集上取得更低的 MSE；其二，该优势是否在波动较强、模式更复杂的数据集上更加显著；其三，在 MSE 改善的同时，MAE 是否保持可接受水平。若结果满足上述趋势，则可以据此说明，小波包分解通过显式拆分不同频带并进行选择性融合，确实增强了模型对尖峰和局部突变的感知能力\cite{hyndman2006accuracy,coifman1992bestbasis}。  
...
为考察小波包分解粒度对模型性能的影响，本节在保持其余结构与训练设置不变的前提下，对分解层数 \(J\) 进行消融。典型设置可包括 \(J=1,2,3\)，分别对应 2、4、8 个叶子子带。通过比较不同层数下的 MSE、MAE、训练时间和参数量，可以分析更细的频带划分是否一定带来更好的预测效果\cite{coifman1992bestbasis,hyndman2006accuracy,chai2014rmsemae}。  
...
从实验设计上看，本章通过主结果表、极端波动场景可视化以及分解层数消融三个角度，对小波包增强模块的作用进行了系统验证。若实验结果表明其在 MSE 上具有更稳定的优势，则可以据此说明：多尺度局部频带建模确实有助于抑制少量高误差样本，并为第六章进一步分析小波包方法与 MSE 优化之间的内在联系提供方法和证据基础\cite{chai2014rmsemae,donoho1994waveshrink}。
